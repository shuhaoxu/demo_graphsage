...
class BipartiteEdgePredLayer(Layer):
    ...
    Args:
        bilinear_weights: use a bilinear weight for affinity calculation: u^T A v.
            If set to false, it is assumed that input dimensions are the same and the affinity will
            be basedon dot product.
    """
    def __init__(self, input_dim1, input_dim2, placeholders, dropout=False, 
                 act=tf.nn.sigmoid, loss_fn='xent', 
                 neg_sample_weights=1.0, bias=False, bilinear_weights=False, **kwargs):
    ...
    def loss(self, inputs1, inputs2, neg_samples):
        ...
        Args:
            neg_samples: tensor of shape [num_neg_samples x iput_dim2]. Negative samples for all 
    ...
    def _xent_loss(self, inputs1, inputs2, neg_samples, hard_neg_samples=None):
        ...
        loss      = tf.reduce_sum(true_xent) + self.neg_sample_weights * tf.reduce_sum(neg_xent)
        return loss

    def _skipgram_loss(self, inputs1, inputs2, neg_samples, hard_neg_samples=None):
        aff      = self.affinity(inputs1, inputs2)
        neg_aff  = self.neg_cos(inupts1, neg_samples, hard_neg_samples)
        neg_cost = self.log(tf.reduce_sum(tf.exp(neg_aff), axis=1))
