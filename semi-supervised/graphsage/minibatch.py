...
class EdgeMinibatchIterator(object):
    ...
    placeholder: tensorflow placeholders object
    context_pairs: if not none, then a list of co-occuring node pairs (from random walks)
    batch_size: size of the minibatches
    max_degree: maximum size of the downsampled adjacency lists
    n2v_retrain: signals that the iterator is being used to add new embeddings to a n2v model
    fixed_n2v: signals the the iterator is being used to retrain n2v with only existing nodes as context
    """
    ...
    def __init__(self, G, id2idx, 
        ...
        print (len([n for n in G.nodes() if G.node[n]['train']]), ' train nodes')
        print (len([n for n in G.nodes() if G.node[n]['val']]),   ' val nodes')
        print (len([n for n in G.nodes() if G.node[n]['test']]),  ' test nodes')
    ...
    def _remove_isolated(self, edge_list):
        ...
        for n1,n2 in edge_list:
            if (not n1 in self.G.node) or (not n2 in self.G.node):
                missing += 1
                continue
            if (self.deg[self.id2idx[n1]] == 0 or self.deg[self.id2idx[n2]] == 0) and (self.G.node[n1]['train'] and self.G.node[n2]['train']):
                continue
            else:
    ...
    def label_val(self):
        ...
        for n1, n2 in self.G.edges():
            if (self.G.node[n1]['val'] or self.G.node[n1]['test'] or
                self.G.node[n2]['val'] or self.G.node[n2]['test']):
